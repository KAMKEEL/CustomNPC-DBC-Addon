...
public class TransformController {
    ...
    public static void Ascend(Form form) {
        ...
        DBCPacketHandler.Instance.sendToServer(new DBCSetValPacket(CustomNpcPlusDBC.proxy.getClientPlayer(), EnumNBTType.INT, "jrmcSaiRg", (int) rage).generatePacket());
        ...
        if (rage >= 100) { //transform when rage meter reaches 100 (max)
            DBCPacketHandler.Instance.sendToServer(new TransformPacket(Minecraft.getMinecraft().thePlayer, form.getID(), true).generatePacket());
