    ...
    public void updateScreen() {
        ...
        if (JGConfigDBCFormMastery.FM_Enabled) {
            ...
            if (dataClient != null && dataClient.isInCustomForm()) {
                ...
                formTooltip = dataClient.getCurrentForm().menuName + " §8Mastery Lvl: §4" + formatter.format(curLevel);
                //DBC Form handling
            } else {
        ...
        dynamicLabels.get("level").updateDisplay(JRMCoreH.numSep(JRMCoreH.getPlayerLevel(JRMCoreH.PlyrAttrbts))).updateTooltip(isMaxLevel ? JRMCoreH.trl("jrmc", "LevelMax") : JRMCoreH.trl("jrmc", "LevelNext", JRMCoreH.cllr + JRMCoreH.attrLvlNext(JRMCoreH.PlyrAttrbts) + JRMCoreH.cldgy));


        dynamicLabels.get("tp").updateDisplay(JRMCoreH.numSep(JRMCoreH.curTP)).updateTooltip(JRMCoreH.cllr + JRMCoreH.numSep(JRMCoreH.attrCst(JRMCoreH.PlyrAttrbts, 0)) + JRMCoreH.cldgy);

        dynamicLabels.get("race").updateDisplay(JRMCoreH.trl("jrmc", JRMCoreH.Races[dbcClient.Race]));
        ...
        dynamicLabels.get("form").updateDisplay(formNameColor + formName).setTooltip(formTooltip);

        dynamicLabels.get("class").updateDisplay(JRMCoreH.trl("jrmc", JRMCoreH.ClassesDBC[dbcClient.Class])).updateTooltip(JRMCoreH.trl("jrmc", JRMCoreH.ClassesDBCDesc[JRMCoreH.Class]));

        dynamicLabels.get("alignment").updateDisplay(JRMCoreH.algnCur(JRMCoreH.align)).updateTooltip(JRMCoreH.align + "%");
...
//            wStr = JRMCoreH.weightPerc(0);
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (originalStatVal != modifiedStatVal) {
                attributeDesc += "\n" + JRMCoreH.trl("jrmc", "Modified") + ": §4" + darkFormColor + statDisplay + "\n§8" + JRMCoreH.trl("jrmc", "Original") + ": §4" + JRMCoreH.numSep(originalStatVal) + "§8";
            ...
            dynamicLabels.get("attr_" + i).updateDisplay((isSTRDEXWIL ? formStatColor : "") + statDisplay).updateTooltip(attributeDesc);
        ...
        dynamicLabels.get("upgradeAmount").updateTooltip(upgradeDescription).updateDisplay(JRMCoreH.clbe + (upgradeCost <= 0 ? JRMCoreH.trl("jrmc", "LimitReached") : (allMaxed ? JRMCoreH.trl("jrmc", "AttributeAllMaxed") : JRMCoreH.numSep(upgradeCost) + " TP " + (upgradeCounter > 0 ? "x" + JRMCoreH.attributeMultiplier(this.upgradeCounter) : "")))).tooltipWidth = descriptionWidth;


        this.dynamicLabels.get("release").updateDisplay(dbcClient.Release);
        ...
        dynamicLabels.get("melee").updateDisplay(formStatColor + JRMCoreH.numSep(longValue)).setTooltip(getDescription(JRMCoreH.attrNms(1, 0), incrementVal, JRMCoreH.numSep(stat), null, null, (bonusOutput > 0 ? JRMCoreH.numSep(bonusOutput) : null), 0, (int) (100.0F - JRMCoreH.weightPerc(0) * 100.0F)));
        ...
        String defDesc = getDescription(JRMCoreH.attrNms(1, 1), incrementVal, JRMCoreH.numSep(stat), passiveDef, chargingDef, (bonusOutput > 0 ? JRMCoreH.numSep(bonusOutput) : null), 0, (int) (100.0F - JRMCoreH.weightPerc(1) * 100.0F));
        dynamicLabels.get("defense").updateDisplay(formStatColor + JRMCoreH.numSep(longValue)).setTooltip(defDesc);


        dynamicLabels.get("passive").updateDisplay(formStatColor + passiveDef).setTooltip(defDesc);


        if (dynamicLabels.get("charging") != null) {
            dynamicLabels.get("charging").updateDisplay(formStatColor + chargingDef).setTooltip(defDesc);
        }
        ...
        dynamicLabels.get("body").updateDisplay(JRMCoreH.numSep(stat) + (isReductionWorthDisplaying ? " R" + dmgReduction + "%" : "")).setTooltip(getDescription(JRMCoreH.attrNms(1, 2), incrementVal, null, null, null, null, (isReductionWorthDisplaying ? dmgReduction : 0), 0));
        ...
        dynamicLabels.get("actionTime").updateDisplay(JRMCoreH.numSep(stat)).setTooltip(getDescription(JRMCoreH.attrNms(1, 2), incrementVal, null, null, null, null, 0, 0));
        ...
        dynamicLabels.get("kiPower").updateDisplay(formStatColor + JRMCoreH.numSep(curAtr)).setTooltip(getDescription(JRMCoreH.attrNms(1, 3), incrementVal, JRMCoreH.numSep(stat), null, null, null, 0, 0));
        ...
        dynamicLabels.get("maxKi").updateDisplay(JRMCoreH.numSep(stat)).setTooltip(getDescription(JRMCoreH.attrNms(1, 5), incrementVal, null, null, null, (bonusOutput > 0 ? JRMCoreH.numSep(bonusOutput) : null), 0, 0));
        ...
        dynamicLabels.get("running").updateDisplay(stat).setTooltip(statDesc);
        ...
        dynamicLabels.get("flying").updateDisplay(stat).setTooltip(statDesc);
    ...
    public void initGui() {
        ...
        dynamicLabels.put("level", new JRMCoreLabel(JRMCoreH.trl("jrmc", "Level") + ": " + (ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + "%s", "%s", guiWidthOffset + 6, guiHeightOffset + index * 10 + 6));
        index++;

        dynamicLabels.put("tp", new JRMCoreLabel(JRMCoreH.trl("jrmc", "TP") + ": " + (ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + "%s", JRMCoreH.trl("jrmc", "TrainingPoints") + "\n" + JRMCoreH.trl("jrmc", "RequiredTP"), guiWidthOffset + 6, guiHeightOffset + index * 10 + 6));
        index++;

        dynamicLabels.put("race", new JRMCoreLabel(JRMCoreH.trl("jrmc", "Race") + ": " + (ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + "%s", null, guiWidthOffset + 6, guiHeightOffset + index * 10 + 6));
        genderIcon = new GuiIcon(icons3, 0, guiHeightOffset + 2 + index * 10 + 1, 0, (JRMCoreH.dnsGender(JRMCoreH.dns) < 1 ? 128 : 112), 16, 16);
        this.hoverableStaticLabels.add(genderIcon);
        index++;

        dynamicLabels.put("form", new JRMCoreLabel(JRMCoreH.trl("jrmc", "TRState") + ": " + (ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + "%s", null, guiWidthOffset + 6, guiHeightOffset + index * 10 + 6

        ));
        index++;

        dynamicLabels.put("class", new JRMCoreLabel(JRMCoreH.trl("jrmc", "Class") + ": " + (ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + "%s", "%s", this.guiWidthOffset + 6, this.guiHeightOffset + index * 10 + 6));
        index++;

        dynamicLabels.put("alignment", new JRMCoreLabel(JRMCoreH.trl("jrmc", "Alignment") + ": " + (ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + "%s", JRMCoreH.trl("jrmc", "AlignmentDesc"), this.guiWidthOffset + 6, this.guiHeightOffset + index * 10 + 6));
        index++;

        hoverableStaticLabels.add(new JRMCoreLabel(JRMCoreH.trl("jrmc", "Attributes") + ":", "%s", this.guiWidthOffset + 6, this.guiHeightOffset + index * 10 + 11).updateDisplay());
        index++;

        String[] statNames = new String[]{"STR", "DEX", "CON", "WIL", "MND", "SPI"};
        ...
        for (int i = 0; i < 6; i++) {
            ...
            dynamicLabels.put("attr_" + i, new JRMCoreLabel((ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + LocalizationHelper.getLocalizedString("statsheet.attribute." + statNames[i]) + ": §4%s", "%s", guiWidthOffset + 17, yPos + 5));
        ...
        dynamicLabels.put("upgradeAmount", new JRMCoreLabel((ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + "UC: %s", "%s", guiWidthOffset + 17, guiHeightOffset + index * 10 + 14).setDisplay("Testing"));

        index = 0;
        this.dynamicLabels.put("release", new JRMCoreLabel(JRMCoreH.trl("jrmc", "PowerRelease") + ": " + (ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + "%s%%", null, guiWidthOffset + 133, guiHeightOffset + index * 10 + 6));
        index++;

        this.hoverableStaticLabels.add(new JRMCoreLabel(null, null, guiWidthOffset + 133, guiHeightOffset + index * 10 + 11).setDisplay(JRMCoreH.trl("jrmc", "Stats")));
        index++;


        this.dynamicLabels.put("melee", new JRMCoreLabel((ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + JRMCoreH.trl("jrmc", "mleDB") + ": §4%s", JRMCoreH.trl("jrmc", "StatIncreaseDesc") + JRMCoreH.trl("jrmc", "StatIncreaseDesc2"), guiWidthOffset + 133, guiHeightOffset + index * 10 + 11));
        index++;

        this.dynamicLabels.put("defense", new JRMCoreLabel((ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + JRMCoreH.trl("jrmc", "DefDB") + ": §4%s", "%s", guiWidthOffset + 133, guiHeightOffset + index * 10 + 11));
        index++;

        this.dynamicLabels.put("passive", new JRMCoreLabel((ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + JRMCoreH.trl("jrmc", "Passive") + ": §4%s", "%s", guiWidthOffset + 138, guiHeightOffset + index * 10 + 11));
        index++;

        if (ClientCache.hasChargingDex) {
            this.dynamicLabels.put("charging", new JRMCoreLabel((ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + LocalizationHelper.getLocalizedString("statsheet.stat.charging") + ": §4%s", "%s", guiWidthOffset + 138, guiHeightOffset + index * 10 + 11));
            index++;
        }

        this.dynamicLabels.put("body", new JRMCoreLabel((ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + JRMCoreH.trl("jrmc", "BdDB") + ": §4%s", "%s", guiWidthOffset + 133, guiHeightOffset + index * 10 + 11));
        index++;

        this.dynamicLabels.put("actionTime", new JRMCoreLabel((ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + JRMCoreH.trl("jrmc", "StDB") + ": §4%s", "%s", guiWidthOffset + 133, guiHeightOffset + index * 10 + 11));
        index++;

        this.dynamicLabels.put("kiPower", new JRMCoreLabel((ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + JRMCoreH.trl("jrmc", "EnPwDB") + ": §4%s", "%s", guiWidthOffset + 133, guiHeightOffset + index * 10 + 11));
        index++;

        this.dynamicLabels.put("maxKi", new JRMCoreLabel((ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + JRMCoreH.trl("jrmc", "EnPlDB") + ": §4%s", "%s", guiWidthOffset + 133, guiHeightOffset + index * 10 + 11));
        index++;
        this.dynamicLabels.put("running", new JRMCoreLabel((ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + JRMCoreH.trl("jrmc", "SpDB") + ": §4%s%%", "%s", guiWidthOffset + 133, guiHeightOffset + index * 10 + 11));
        index++;
        this.dynamicLabels.put("flying", new JRMCoreLabel((ConfigDBCClient.DarkMode ? DARKMODE_ACCENT : "") + JRMCoreH.trl("jrmc", "FSDB") + ": §4%s%%", "%s", guiWidthOffset + 133, guiHeightOffset + index * 10 + 11));
        index++;
