...
public class SubGuiKaiokenDrain extends SubGuiInterface implements ITextfieldListener {
    ...
    public void unFocused(GuiNpcTextField guiNpcTextField) {
        ...
        if (id == 1) {
            kaioStackable.setKaioDrain(guiNpcTextField.getFloat());
        }
        if (id >= 3 && id <= 8) {
            kaioStackable.setKaioState2Balance(id - 3, editStrained, guiNpcTextField.getFloat());
        }
    ...
    public void actionPerformed(GuiButton button) {
        if (button.id == 11) {
            kaioStackable.kaiokenMultipliesCurrentFormDrain = !kaioStackable.kaiokenMultipliesCurrentFormDrain;
            initGui();
        }
        if (button.id == 2) {
