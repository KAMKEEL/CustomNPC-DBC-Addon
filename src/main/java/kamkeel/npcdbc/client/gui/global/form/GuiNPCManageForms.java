...
public class GuiNPCManageForms extends GuiNPCInterface2 implements ICustomScrollListener, IScrollData, IGuiData, ISubGuiListener, GuiYesNoCallback {
    ...
    public GuiNPCManageForms(EntityNPCInterface npc) {
        ...
        DBCPacketHandler.Instance.sendToServer(new DBCRequestForm(-1, false,false).generatePacket());
    }
    ...
    protected void actionPerformed(GuiButton guibutton) {
        ...
        if (button.id == 0) {
            ...
            DBCPacketHandler.Instance.sendToServer(new DBCSaveForm(form.writeToNBT(), name).generatePacket());
        }
        if (button.id == 1) {
        ...
        if (button.id == 3) {
            ...
            while (data.containsKey(form.name))
                form.name += "_";
            DBCPacketHandler.Instance.sendToServer(new DBCSaveForm(form.writeToNBT(), form.name).generatePacket());
        }
    ...
    public void customScrollClicked(int i, int j, int k, GuiCustomScroll guiCustomScroll) {
        if (guiCustomScroll.id == 0) {
            ...
            if (selected != null && !selected.isEmpty())
                DBCPacketHandler.Instance.sendToServer(new DBCGetForm(data.get(selected)).generatePacket());
        }
    }
    ...
    public void confirmClicked(boolean result, int id) {
        ...
        if (id == 1) {
            if (data.containsKey(scrollForms.getSelected())) {
                DBCPacketHandler.Instance.sendToServer(new DBCRemoveForm(data.get(scrollForms.getSelected())).generatePacket());
                scrollForms.clear();
                form = new Form();
